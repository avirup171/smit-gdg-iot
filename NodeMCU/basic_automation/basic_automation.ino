...
void callback(char* topic, byte* payload, unsigned int length) 
{
  ...
  if (msgString == "L1ON"){    // if there is a "1" published to any topic (#) on the broker then:
    digitalWrite(16, LOW);     // set pin to the opposite state 
    Serial.println("Switching L1 ON"); 
  }
  else if(msgString == "L1OFF")
  {
  
    digitalWrite(16, HIGH);     // set pin to the opposite state 
